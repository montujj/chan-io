...
def ui(backend=None):
    ...
    if backend is None:
        ...
        if backend is None:
            try:
                import tde4
                # TO DO: Change this to tde4_io when that module is implemented, and update
    ...
    if backend is None:
        raise RuntimeError(
            "Could not detect supported DCC environment (Maya, Nuke, or 3DEqualizer). Please specify "
            "backend explicitly."
        )
