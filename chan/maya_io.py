...
class MayaChanBackend(ChanDCCInterface):
    ...
    # This backend requires a shape node (camera or mesh) to determine if it's exporting a camera 
    # or regular transform, which affects the .chan format.
    REQUIRE_SHAPE = True

    def export_chan(self, path: str, node: str, shape: str, start: int, end: int) -> int:
        ...
        Args:
            ...
            shape (str or None): Name of the shape node (camera or mesh) if present, otherwise None.
        ...
        for frame in range(start, end + 1):
            ...
            translate_xyz = cmds.xform(node, query=True, worldSpace=True, translation=True)
            rotate_xyz = cmds.xform(node, query=True, worldSpace=True, rotation=True)
            tx, ty, tz = translate_xyz
            rx, ry, rz = rotate_xyz
            if is_camera:
                focal = cmds.getAttr(f"{shape}.focalLength")
                h_aperture = cmds.getAttr(f"{shape}.horizontalFilmAperture") * 25.4  # inches to mm
                v_aperture = cmds.getAttr(f"{shape}.verticalFilmAperture") * 25.4  # inches to mm
            else:
                focal = 0.0
                h_aperture = 0.0
                v_aperture = 0.0
            ...
            lines.append(
                f"{tx:.6f} {ty:.6f} {tz:.6f} {rx:.6f} {ry:.6f} {rz:.6f} {focal:.6f} {h_aperture:.6f} {v_aperture:.6f}"
            )
    ...
    def import_chan(self, path: str, node: str) -> int:
        ...
        for i, line in enumerate(lines):
            ...
            if is_camera and len(vals) >= 9:
                ...
                if i == 0:
                    cmds.setAttr(f"{shape}.horizontalFilmAperture", h_aperture / 25.4)
                    cmds.setAttr(f"{shape}.verticalFilmAperture", v_aperture / 25.4)
    ...
    def get_selected(self):
        """Get the first selected transform node and check if it has a camera shape or mesh shape."""
    ...
    def get_playback_range(self):
        ...
        Returns:
            tuple[int, int]: The start and end frames of the current playback range.
        """
